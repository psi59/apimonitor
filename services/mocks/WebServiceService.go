...
// GetWebServiceList provides a mock function with given fields: transaction, request
func (_m *WebServiceService) GetWebServiceList(transaction rsdb.Transaction, request models.WebServiceListRequest) (*rsmodel.PaginatedList, *amerr.ErrorWithLanguage) {
	ret := _m.Called(transaction, request)

	var r0 *rsmodel.PaginatedList
	if rf, ok := ret.Get(0).(func(rsdb.Transaction, models.WebServiceListRequest) *rsmodel.PaginatedList); ok {
		r0 = rf(transaction, request)
	} else {
		if ret.Get(0) != nil {
	...
	var r1 *amerr.ErrorWithLanguage
	if rf, ok := ret.Get(1).(func(rsdb.Transaction, models.WebServiceListRequest) *amerr.ErrorWithLanguage); ok {
		r1 = rf(transaction, request)
	} else {
		if ret.Get(1) != nil {
			r1 = ret.Get(1).(*amerr.ErrorWithLanguage)
